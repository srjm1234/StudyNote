...
**1. WinForms 项目由 \_\_\_\_ 和 \_\_\_\_ 两个部分文件组成（partial class）。**

> [!success]- 答案
> `Form1.cs`（业务逻辑）；`Form1.Designer.cs`（界面设计代码）。用 partial 关键字把类拆到多个文件。

**2. 窗体打开时自动执行的事件是 \_\_\_\_，关闭时是 \_\_\_\_。**

> [!success]- 答案
> `Load`（窗体加载）；`FormClosing`（关闭中，可取消）/ `FormClosed`（已关闭）。

**3. 三个常用容器控件：\_\_\_\_、\_\_\_\_、\_\_\_\_。**

> [!success]- 答案
> Panel、GroupBox、TabControl。

**4. 定时器控件是 \_\_\_\_，通过 \_\_\_\_ 事件周期触发。**

> [!success]- 答案
> `Timer`；`Tick`。常用作倒计时、轮询。

**5. 多线程中更新 UI 必须用 \_\_\_\_ 方法，把操作封送回 UI 线程。**

> [!success]- 答案
> `BeginInvoke`（或 Invoke）。

**6. Socket 收发的是 \_\_\_\_ 类型数据，发送前用 \_\_\_\_ 编码，接收后用 \_\_\_\_ 解码。**
