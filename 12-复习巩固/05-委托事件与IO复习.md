...
**1. Lambda 表达式的基本格式：\_\_\_\_，如 `v => v % 2 == 0`。**

> [!success]- 答案
> `(参数列表) => { 方法体 }` 或简化 `参数 => 表达式`（单表达式可省略大括号和 return）。

**2. 内置委托类型：无返回值用 \_\_\_\_，有返回值用 \_\_\_\_。**

> [!success]- 答案
> `Action<参数...>`；`Func<参数..., 返回值>`。

**3. 多播委托用 \_\_\_\_ 添加方法、\_\_\_\_ 移除方法。**

> [!success]- 答案
> `+=`；`-=`。调用时按添加顺序依次执行所有方法。

**4. 文件路径基础：以盘符开头的是 \_\_\_\_ 路径，`./` 表示 \_\_\_\_，`../` 表示 \_\_\_\_。**

> [!success]- 答案
> 绝对路径；当前目录；上一级目录。

**5. `BinaryWriter.Write(12345)` 写入的是 \_\_\_\_ 类型数据，读取时用 \_\_\_\_ 方法。**

> [!success]- 答案
> int（整数）；`ReadInt32()`。写入和读取的顺序、类型必须一致。

**6. 文本文件读写推荐用 \_\_\_\_ 类，二进制用 \_\_\_\_ 类，底层字节流用 \_\_\_\_ 类。**
